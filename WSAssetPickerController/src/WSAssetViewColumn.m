...
@property (nonatomic, strong) WSAssetWrapper* assetWrapper;
...
{
    WSAssetViewColumn *assetView = [[WSAssetViewColumn alloc] initWithImage:thumbnail andData:assetWrapper];
...
- (id)initWithImage:(UIImage *)thumbnail andData:(WSAssetWrapper*)assetWrapper
{
    if ((self = [super initWithFrame:ASSET_VIEW_FRAME])) {
        ...
        if ([[[self.assetWrapper.asset defaultRepresentation] UTI] isEqualToString:(__bridge NSString*)kUTTypeGIF]) {
            NSLog(@"url = %@", [[self.assetWrapper.asset defaultRepresentation] url]);
            dispatch_async(dispatch_get_global_queue(0, 0), ^{
                ALAssetRepresentation *rep = [self.assetWrapper.asset defaultRepresentation];
                Byte *buffer = (Byte*)malloc(rep.size);
                NSUInteger buffered = [rep getBytes:buffer fromOffset:0.0 length:rep.size error:nil];
                NSData *data = [NSData dataWithBytesNoCopy:buffer length:buffered freeWhenDone:YES];//this is NSData may be what you want
                dispatch_async(dispatch_get_main_queue(), ^{
                    YLImageView* imgView = [[YLImageView alloc] initWithFrame:ASSET_VIEW_FRAME];
                    imgView.contentMode = UIViewContentModeScaleAspectFill;
                    imgView.clipsToBounds = YES;
                    imgView.image = [[YLGIFImage alloc] initWithData:data];
